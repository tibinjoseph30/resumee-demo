...
const CreateCourse: React.FC<CourseCreateProps> = ({ onCancel }) => {
    const [startDate, setStartDate] = useState<Date | null>(new Date());
    return (
        <div>
            <div className="mb-8">
                <div className="text-2xl font-semibold">Create new course</div>
                <div className="text-slate-400 mt-1">Fill up the details below</div>
            </div>
            <div className="grid grid-cols-2 gap-7">
                <div className="form-group col-span-2">
                    <label htmlFor="" className="control-label">Course</label>
                    <input type="text" placeholder="eg: animation" className="control border-2 p-4 rounded-md" />
                </div>
