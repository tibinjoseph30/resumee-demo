...
const ResumePreview = () => {
  ...
  const renderLayout = (layout: string, font: string, color: string) => {
    ...
    switch (layout) {
      case "standard":
        return (
          <div className={fontClass} style={layoutStyle}>
            <StandardLayout />
          </div>
        );
      case "singlePage":
  ...
  return (
    <div>
      <Formik initialValues={previewInitialValues} onSubmit={handleSubmit}>
        {({ setFieldValue, handleBlur, values, resetForm }) => (
          <div className="grid lg:grid-cols-3 sm:py-6 py-3">
            <div
              onClick={() => setIsOpen(true)}
              className={`bg-primary text-white rounded-full flex items-center justify-center fixed ${styles.settingsToggler}`}
            >
              <HiCog size={24} />
            </div>
            <div
              onClick={() => setIsOpen(false)}
              className={`inset-0 bg-slate-900/50 ${
                isOpen ? "fixed" : "hidden"
              }`}
            ></div>
            <div className="col-span-2 overflow-x-auto">
              <div
                className={`bg-white p-6 sm:w-4/5 sm:mx-auto mx-3 shadow-sm ${styles.preview}`}
              >
                {renderLayout(
                  values.layouts,
                  values.fonts,
            ...
            <div
              className={`bg-white fixed lg:w-1/3 top-0 right-0 bottom-0 p-6 border-l ${
                styles.settingsController
              } ${isOpen ? styles.opened : ""}`}
            >
              <Form>
                <div className="grid gap-6">
                  <div className="form-group">
                    <label htmlFor="layouts" className="control-label">
                      Layout
                    </label>
                    <Field name="layouts">
                  ...
                  <div className="form-group">
                    <label htmlFor="fonts" className="control-label">
                      Font
                    </label>
                    <Field name="fonts">
                      {({ field, form }: { field: any; form: any }) => (
                  ...
                  <div className="form-group">
                    <label htmlFor="colorPicker" className="control-label">
                      Color
                    </label>
                    <Field
                      type="color"
                ...
                <button className="w-full flex justify-center bg-primary text-white p-5 rounded-md mt-6 hover:bg-primary">
                  Download PDF
                </button>
