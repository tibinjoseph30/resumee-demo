    ...
    <div>
      ...
      >
        <Form className="grid gap-7">
          ...
          <button
            type="submit"
            className="flex items-center justify-center gap-2 bg-primary p-4 text-white font-medium rounded-md hover:bg-primary"
            disabled={loading}
          >
            {loading ? (
              <>
      ...
      <div className="mt-4 text-center text-slate-500">
        Not registered? <Link href="/sign-up">Create an account</Link>
      </div>
      {error && (
        <div className="p-4 text-center rounded-md bg-yellow-600/[0.1] text-yellow-700 text-sm mt-4">
          {error}
        </div>
      )}
    </div>
